...
from web.utils import send_email, get_md5, save_user_avatar
...
from web.blog_signals import oauth_user_login_signal
...
def authorize(request):
    ...
    if user:
        if not user.nikename.strip():
            import datetime
            user.nikename = "web" + datetime.datetime.now().strftime('%y%m%d%I%M%S')
        try:
...
def emailconfirm(request, id, sign):
    ...
    if oauthuser.author:
        ...
        if result[1]:
            ...
            author.username = oauthuser.nikename.strip() if oauthuser.nikename.strip() else "web" + datetime.datetime.now().strftime(
                '%y%m%d%I%M%S')
            author.save()
